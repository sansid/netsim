  ...
  if ( (test == 1) || (test == 3) )
    {
      ...
      clientHelper1.SetAttribute 
        ("PacketSize", UintegerValue (1460));//1000

      ApplicationContainer clientApps1;
      AddressValue remoteAddress
        (InetSocketAddress (i2i3.GetAddress (1), port));
      clientHelper1.SetAttribute ("Remote", remoteAddress);
      clientApps1.Add (clientHelper1.Install (n2n3.Get (0)));
  ...
  else // 4 or 5
    {
      ...
      clientHelper1.SetAttribute 
        ("DataRate", DataRateValue (DataRate ("10Mb/s")));
      clientHelper1.SetAttribute 
        ("PacketSize", UintegerValue (1460));

      ApplicationContainer clientApps1;
      AddressValue remoteAddress1
        (InetSocketAddress (i2i3.GetAddress (1), port1));
      ...
      AddressValue remoteAddress2
        (InetSocketAddress (i2i3.GetAddress (0), port2));
      clientHelper2.SetAttribute ("Remote", remoteAddress2);
      clientApps2.Add (clientHelper2.Install (n2n3.Get (1)));
      clientApps2.Start (Seconds (3.5));
...
main (int argc, char *argv[])
{
  ...
  uint32_t redTest;
  std::string redLinkDataRate = "20Mbps";
  std::string redLinkDelay = "100ms";
  ...
  NodeContainer c;
  c.Create (2);
  Names::Add ( "N2", c.Get (0));
  Names::Add ( "N3", c.Get (1));
  n2n3 = NodeContainer (c.Get (0), c.Get (1));

  Config::SetDefault ("ns3::TcpL4Protocol::SocketType", StringValue ("ns3::TcpNewReno"));
  // 42 = headers size
  Config::SetDefault ("ns3::TcpSocket::SegmentSize", UintegerValue (1460)); //42 bytes header
