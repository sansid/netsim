/**************************************************
* File : sample file for running red tests.
*
*************************************************** */
...
//set configuration for DropTailQueue, RedQUeue and TCPSocket
  ...
  tchPfifo.AddInternalQueues (handle, 3, "ns3::DropTailQueue", "MaxBytes", UintegerValue (maxBytes));
  tchPfifo.AddInternalQueues (handle, 3, "ns3::DropTailQueue", "Mode", StringValue ("QUEUE_MODE_BYTES"));
...
Config::SetDefault ("ns3::RedQueueDisc::MinTh", DoubleValue (minimum_threshold));
Config::SetDefault ("ns3::RedQueueDisc::MaxTh", DoubleValue (maximum_threshold));
Config::SetDefault ("ns3::RedQueueDisc::QW", DoubleValue (Wq));
Config::SetDefault ("ns3::RedQueueDisc::QueueLimit", UintegerValue (qlen));
Config::SetDefault ("ns3::RedQueueDisc::LInterm", DoubleValue (maxP));
Config::SetDefault ("ns3::TcpSocket::RcvBufSize", UintegerValue(window_size));


std::string type;//define a string
type = "ns3::RedQueueDisc"; //change to DropTailQueue if needed
//type = "ns3::DropTail"; //change to DropTailQueue if needed
...
//tcpPfifo.Install(device_bottleneck_link2);
